...
export const UploadModal: React.FC<UploadModalProps> = ({ isOpen, onClose, defaultPo }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to upload and parse document.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 relative">
        ...
        {error && <div className="p-3 mb-4 text-xs bg-rose-50 text-rose-700 border border-rose-200 rounded-lg">{error}</div>}
